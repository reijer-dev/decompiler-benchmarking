...
public class Assessor {
    ...
    static {
        ...
        s_sbDummyDecompiledCFile.append("/* This is a dummy file, automatically generated by the assessor.\n");
        s_sbDummyDecompiledCFile.append("   It was generated, because the decompiler was unable to produce\n");
        ...
        for (int iTestNumber = 0; iTestNumber < iNumberOfTests; ++iTestNumber) {
            ...
            for(var config : CompilerConfig.configs){
                ...
                tasks.add(() -> {
                    ...
                    // make dummy decompiler output file if the real one does not exist.
                    //
                    // why?
                    //
                    // well. Say we have 10 test binaries. The container score is the
                    // average of those 10. Except: if we simply skip the ones where no
                    // decompiled C is found, the result would be the average of, say, only
                    // 7 tests. This is wrong: it should still be an average of 10 tests,
                    // with 3 tests having 0% scores.
                    //
                    // This is why we make a dummy file, which only contains int main(int argc, char *argv[]){}.
                    // No assessor will find anything here. We mark the file as a dummy file, so assessors know what
                    // they are given
                    //
                    if (!bFileExists(strCDest)) {
                        // no output file exists, so we create our dummy one
                        IOElements.writeToFile(s_sbDummyDecompiledCFile, strCDest);
                        codeinfo.bDummyFile = true;
                    }
                    // now we are sure we have an output file,
                    // so we don't need to test for its existence when setting up the actual assessing
