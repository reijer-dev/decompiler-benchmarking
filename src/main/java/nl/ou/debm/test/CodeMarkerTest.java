...
import static nl.ou.debm.common.CodeMarker.STRCODEMARKERGUID;
...
//        assertNotNull(cm.getID());  TODO
    ...
    @Test
    void TestMarkerInLLVM(){
        final String STRTESTTEXT = "@.str = private unnamed_addr constant [113 x i8] c\"c5852db2-7acb-cba3-7f81-e7ef3cd1d3b8FF>>functionName:FF_function_3,ID:150b,autoGeneratedFunctionStartMarker:true\\00\", align 1";

        CodeMarker cm = CodeMarker.findInGlobalDef(EFeaturePrefix.FUNCTIONFEATURE, STRTESTTEXT);
        assertNotNull(cm);
        assertInstanceOf(FunctionCodeMarker.class, cm);

        cm = CodeMarker.findInGlobalDef(EFeaturePrefix.CONTROLFLOWFEATURE, STRTESTTEXT);
        assertNull(cm);
